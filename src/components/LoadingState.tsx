...
export default function LoadingState({
  ...
  const labelEl = (
    <span
      ...
      style={{
        ...
        animation: reduced ? "none" : "shimmer-text 1.4s linear infinite",
      }}
    >
      {resolvedLabel}
    </span>
    ...
    return (
      <div role="status" className="flex w-fit flex-col items-start">
        <div className="flex items-center gap-2.5">
          <LoaderGrid {...PATTERNS.Drive} reduced={reduced} />
          {labelEl}
          {elapsedEl}
        </div>
        <div
  ...
  return (
    <div role="status" className="flex w-fit items-center gap-2.5">
      <LoaderGrid delays={delays} dur={dur} round={round} reduced={reduced} />
      {labelEl}
      {elapsedEl}
    </div>
  );
